...
from screener.modules.market.screening.swing import MultiSetupSwingStrategy, SwingScreeningConfig
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    pipeline = DailyWatchlistPipeline(
        ...
        IndicatorService(),
        CandidateScanner(ScreeningEngine(MultiSetupSwingStrategy(screening_config))),
        SwingCandidateRanker(screening_config),
    ...
    if should_start_scheduler():
        scheduler.start()
        logger.info("daily watchlist scheduler started")
    try:
        yield
    finally:
        if scheduler.running:
            scheduler.shutdown(wait=False)
            logger.info("daily watchlist scheduler stopped")
        await client.aclose()
