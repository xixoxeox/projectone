...
async def test_save_one_day_orders_entries_and_exists(postgres_session: AsyncSession) -> None:
    repository = WatchlistRepository(postgres_session)
    day = date(2026, 7, 24)
...
async def test_multiple_days_latest_and_delete_are_date_scoped(
...
async def test_latest_is_empty_when_repository_is_empty(postgres_session: AsyncSession) -> None:
    assert await WatchlistRepository(postgres_session).latest() == []
...
async def test_invalid_candidates_are_rejected_before_writing(
    postgres_session: AsyncSession, values: list[RankedCandidate], message: str
) -> None:
    repository = WatchlistRepository(postgres_session)
...
async def test_empty_trading_date_is_rejected(postgres_session: AsyncSession) -> None:
    repository = WatchlistRepository(postgres_session)
    with pytest.raises(ValueError, match="trading_date"):
        await repository.save(None, [])  # type: ignore[arg-type]


async def test_snapshot_and_all_decimals_preserve_precision(
    postgres_session: AsyncSession,
) -> None:
    repository = WatchlistRepository(postgres_session)
...
async def test_failed_save_rolls_back_delete_and_partial_insert(
    postgres_session: AsyncSession, monkeypatch: pytest.MonkeyPatch
) -> None:
    repository = WatchlistRepository(postgres_session)
    day = date(2026, 7, 24)
