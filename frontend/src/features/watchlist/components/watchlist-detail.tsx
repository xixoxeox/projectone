...
export function WatchlistDetail({ tradingDate, symbol }: { tradingDate: string; symbol: string }) {
  ...
  return <main className="watchlist-shell detail"><Link className="back-link" href={`/watchlist?date=${encodeURIComponent(tradingDate)}`}>← 목록으로</Link><header className="detail-hero"><p>{detail.trading_date}</p><div><span className="detail-rank">#{detail.rank}</span><h1>{detail.symbol}</h1></div><p className="detail-score"><span>종합 점수</span>{formatDecimal(detail.total_score)}</p></header>
    {(detail.screener_name || detail.primary_setup) && <section className="detail-section" aria-labelledby="setup-heading"><h2 id="setup-heading">멀티 셋업</h2><p>{detail.screener_name} v{detail.screener_version}</p><p>주요 셋업: {detail.primary_setup ?? "—"}</p><p>일치 셋업: {(detail.matched_setups ?? []).join(", ") || "—"}</p><dl className="detail-grid">{Object.entries(detail.setup_scores ?? {}).map(([key,value])=><div key={key}><dt>{key}</dt><dd>{formatDecimal(value)}</dd></div>)}</dl></section>}
    ...
    {Object.entries(detail.setup_metrics ?? {}).map(([setup,metrics])=><section className="detail-section" key={setup}><h2>{setup} 지표</h2><dl className="detail-grid">{Object.entries(metrics).map(([key,value])=><div key={key}><dt>{key}</dt><dd>{formatDecimal(value)}</dd></div>)}</dl></section>)}
    {Object.keys(detail.rule_evaluations ?? {}).length>0&&<section className="detail-section"><h2>셋업별 규칙</h2><ul>{Object.entries(detail.rule_evaluations ?? {}).map(([key,value])=><li key={key}>{key}: {value?"통과":"미통과"}</li>)}</ul></section>}
    {Object.keys(detail.configuration_snapshot ?? {}).length>0&&<section className="detail-section"><h2>설정 스냅샷</h2><dl className="detail-grid">{Object.entries(detail.configuration_snapshot ?? {}).map(([key,value])=><div key={key}><dt>{key}</dt><dd>{String(value)}</dd></div>)}</dl></section>}
    <p className="muted">기술적 조건에 따라 선별된 관찰 후보이며 미래 수익이나 매수를 보장하지 않습니다.</p>
